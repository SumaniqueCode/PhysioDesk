...
export default function DesignSystemPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-8 py-12">
      ...
      <Section title="Form fields">
        <div className="grid max-w-3xl grid-cols-1 gap-5 sm:grid-cols-2">
          ...
          <Select
            ...
            defaultValue="suman"
            options={[
              { label: "Dr. Suman Regmi", value: "suman" },
              { label: "Dr. Michelle Voss", value: "michelle" },
            ]}
          />
      ...
      <Section title="Table — toolbar, hover, selected row">
        <TableCard className="max-w-4xl">
          ...
          <Table>
            ...
            <tbody>
              {patients.map((p) => (
                <Tr key={p.name} clickable style={p.selected ? { boxShadow: "inset 3px 0 0 var(--primary)" } : undefined} className={p.selected ? "bg-primary-soft/40" : undefined}>
                  <Td>
        ...
        <Pagination className="mt-4 max-w-4xl" page={page} pageCount={12} totalItems={230} pageSize={20} onPageChange={setPage} />
      </Section>
      ...
      <Section title="Toasts — react-toastify, themed to the palette">
        <div className="flex flex-wrap gap-3">
          <Button variant="success" onClick={() => toast.success("Invoice paid — Rs 4,800 recorded for Aarav Thapa.")}>
            Success toast
          </Button>
          <Button variant="destructive" onClick={() => toast.error("Could not save changes. Check your connection.")}>
            Error toast
          </Button>
          <Button variant="muted" onClick={() => toast.info("Generating schedule — you can edit once it's ready.")}>
            Info toast
          </Button>
        </div>
      </Section>
